...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const dotProduction = x1 * x2 + y1 * y2;
  const magnitude = Math.sqrt(x1 ** 2 + y1 ** 2) * Math.sqrt(x2 ** 2 + y2 ** 2);
  return Math.acos(dotProduction / magnitude);
}
...
function toNumber(value, def) {
  ...
  return !Number.isNaN(n) && typeof n === 'number' ? n : def;
}
...
function getCountOfOddNumbers(number) {
  let total = 0;
  for (let i = 0; i <= Math.abs(number); i += 1) if (i % 2 !== 0) total += 1;
  return total;
}
